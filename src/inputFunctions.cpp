...
void readFitsHeader( const std::string inputFile ,  // Name of the FITS file
  ...
  float fov, redshift, mass, r_vir, c, ba, ca, phi, theta, integ, physicalsize, totMass;
  ...
  try{
    h0->readKey( "INTEG"        , integ        ); printf("41\n");
  } catch (...){
    integ = 0.0;
  }
  ...
  userInput.setNpix       ( N_pixels_h   *
                            N_pixels_v   );
  userInput.setImageMass  ( totMass      );
